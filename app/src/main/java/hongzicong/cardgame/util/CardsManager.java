...
//    }
    ...
    public static int compare(CardsHolder f, CardsHolder s) {
        if(f.getCards().length != s.getCards().length)
            return -1;
        if(getCardNumber(f.getValue()) > getCardNumber(s.getValue())) {
            return 1;
        }
        if(getCardNumber(f.getValue()) == getCardNumber(s.getValue())) {
            if(getImageCol(f.getValue()) < getImageCol(s.getValue())){
                return 1;
            }
            if(getImageCol(f.getValue()) > getImageCol(s.getValue())){
                return 0;
    ...
    public static int[] findBigerCards(CardsHolder card, int cards[]) {
            ...
            int cardType = card.getCardsType();
    ...
    public static void giveCard(int[][] playerCards,int[] cards ){
        for(int i = 0; i < 52; i++) {
